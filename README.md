...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 工具设备保养** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。
...
| 工具设备 | `/equipment`、`/equipment/:id` | 工具设备台账（名称/编号/配置数量/使用人/保养周期）、按周期生成保养提醒（已逾期/临近到期）、保养内容与更换部件登记、损坏报废登记（原因 + 处置方式） |
...
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、设备、保养记录
...
| GET | `/equipment` | 设备分页查询（keyword/category/status/due/restroom_id/排序/分页） |
| POST | `/equipment` | 新增设备，编号留空自动生成 `SB-0001` |
| GET | `/equipment/reminders` | 保养提醒：已逾期与临近到期（默认提前 7 天）两组设备 |
| GET/PATCH/DELETE | `/equipment/{id}` | 详情（含保养记录）/ 更新 / 删除（有保养记录时需 `force=true`） |
| POST | `/equipment/{id}/maintenance` | 登记保养：记录保养内容与更换部件，自动顺延下次保养日期 |
| POST | `/equipment/{id}/scrap` | 报废登记：记录报废原因与处置方式，状态置为「已报废」 |
| GET | `/stats/overview` | 核心指标 |
| GET | `/stats/dashboard` | 看板聚合数据（趋势、分布、区域、排行、最新记录） |
| GET | `/meta/dictionaries` | 枚举字典（状态、分类、程度、检查项、流转规则、设备分类/状态/处置方式） |
...
- **设备编号**：`SB-` + 四位流水号，新增时留空自动生成。
- **保养提醒**：下次保养日期 = 上次保养时间（无保养记录时取购置日期或建档日期）+ 保养周期。下次保养日期早于今天为「已逾期」，7 天内到期为「临近到期」；列表可按到期状态筛选，看板展示提醒卡片与明细。登记保养后自动顺延下次保养日期；修改保养周期会按同一规则重算。
- **报废登记**：报废必须登记报废原因与处置方式（回收/变卖/废弃/其他），登记后状态置为「已报废」，不再参与保养提醒，不可再编辑或登记保养；通过 PATCH 直接改状态为「已报废」会被拒绝。
- **删除保护**：删除公厕时若已存在巡查或问题记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。删除设备时若已有保养记录同样需要 `force=true`。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹、10 台工具设备（覆盖正常/临近到期/已逾期/已报废四种保养状态，含保养记录与报废登记）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。
...
- 后端 `pytest`：11 个用例全部通过；`/health`、台账/巡查/问题/设备/统计/字典接口均返回预期数据。
