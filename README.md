...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 工具设备维护** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。

## 功能模块

| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡、巡查与问题趋势、整改状态/分类/严重程度分布、区域运行情况、重点关注公厕、最新问题与巡查、设备保养提醒 |
...
| 工具设备 | `/equipment`、`/equipment/:id` | 工具设备登记（名称/编号/配置数量/使用人/保养周期）、按周期生成保养提醒、保养内容与更换部件记录、损坏报废登记（原因与处置方式）、删除保护 |

其他页面不会互相混杂：台账、巡查、问题、设备各自独立成页，详情页再做跨模块的关联展示。
...
├── backend
│   ├── app
│   │   ├── api/v1/endpoints      # 路由层：restrooms / inspections / issues / equipment / stats / meta
│   │   ├── core                 # 配置、数据库、业务常量、领域异常
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、设备、保养与报废记录
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、问题整改、设备保养、评分、统计
│   │   ├── seed.py              # 演示数据生成
...
│   │   ├── pages                # dashboard / restrooms / inspections / issues / equipment 五个模块
│   │   ├── utils                # 时间格式化、评分换算
│   │   └── styles/global.css
│   ├── nginx.conf
│   └── Dockerfile
...
| GET | `/equipment` | 工具设备分页查询（keyword/category/status/maintenance_cycle/assignee/due/排序/分页） |
| POST | `/equipment` | 登记工具设备，编号留空自动生成 `GJ-0001` |
| GET | `/equipment/reminders` | 保养提醒：已超期与 7 日内到期两组（`within_days` 可调） |
| GET/PATCH/DELETE | `/equipment/{id}` | 详情（含保养与报废记录）/ 更新 / 删除（有记录时需 `force=true`） |
| POST | `/equipment/{id}/maintenances` | 登记保养：内容、更换部件、费用，自动顺延下次保养日期 |
| POST | `/equipment/{id}/scraps` | 报废登记：数量、原因、处置方式，全部报废后状态转「已报废」 |
| GET | `/stats/overview` | 核心指标 |
| GET | `/stats/dashboard` | 看板聚合数据（趋势、分布、区域、排行、最新记录、设备保养提醒） |
| GET | `/meta/dictionaries` | 枚举字典（状态、分类、程度、检查项、流转规则、设备分类、保养周期、处置方式） |
...
- **设备编号**：`GJ-` + 四位流水号，留空自动生成。
- **保养周期提醒**：下次保养日期 = 最近保养时间（未保养过按建档时间）+ 保养周期（每周 7 天 / 每半月 15 天 / 每月 30 天 / 每季度 90 天 / 每半年 180 天 / 每年 365 天）。登记保养后自动顺延；修改保养周期会重新推算。`下次保养日期 < 当前时间` 为「已超期」，7 天内到期为「即将到期」，列表页顶部、设备详情与总览看板均会给出提醒。
- **报废登记**：每次登记报废数量、原因与处置方式（回收处理/废品变卖/集中销毁/其他），可用数量 = 配置数量 − 累计报废数量；可用为 0 时设备自动转为「已报废」，已报废设备不能再登记保养或报废；补回配置数量可恢复「在用」。
- **删除保护**：删除公厕时若已存在巡查或问题记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；删除设备时若已存在保养或报废记录同理。前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹，以及 11 项工具设备（含历史保养记录、部分报废与全部报废样本，可看到已超期与即将到期的保养提醒）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。

## 测试与验证

```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、流程流转、设备保养与报废、统计）
cd frontend && npm run build     # 生产构建
```

本项目完成时已实际运行验证：

- 后端 `pytest`：13 个用例全部通过；`/health`、台账/巡查/问题/设备/统计/字典接口均返回预期数据。
...
- 工具设备模块端到端验证（Chromium 无头模式，22 项检查全部通过）：看板保养提醒卡片与面板、列表提醒横幅与超期快捷筛选、关键字/周期筛选、列表登记保养（下次保养日期自动顺延、超期标记消除）、详情页档案/保养记录/报废记录三个页签、报废登记与处置方式展示、已报废设备操作禁用、新增设备自动编号、删除设备。全程无控制台报错。
